...
import { Waves, ChevronRight, RefreshCw } from "lucide-react";
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-500/15 ring-1 ring-cyan-500/30">
            <Waves className="h-5 w-5 text-cyan-400" strokeWidth={2} />
            <span className="absolute inset-0 rounded-xl ring-1 ring-cyan-400/20 animate-ping-ring" />
          </div>

          <div className="hidden sm:block">
            <p className="font-display text-sm font-extrabold tracking-widest text-slate-100 leading-none">
              ERK GROUP
            </p>
          ...
          <div className="hidden md:block h-7 w-px bg-slate-700/60 mx-1" />

          <nav
            aria-label="breadcrumb"
            className="hidden md:flex items-center gap-1 text-xs text-slate-400 font-display"
          >
            <span className="text-slate-400 font-medium">Dashboard</span>
            <ChevronRight className="h-3.5 w-3.5 text-slate-600" />
            <span className="text-slate-400 font-medium">Data Sensor</span>
            <ChevronRight className="h-3.5 w-3.5 text-slate-600" />
            <span className="text-cyan-400 font-semibold">Realtime</span>
          </nav>
        </div>
        ...
        <div className="flex items-center gap-3">
          {lastUpdated && (
            <p className="hidden sm:block font-mono-data text-[11px] text-slate-500">
              Update:{" "}
              <span className="text-slate-400">{formatTime(lastUpdated)}</span>
            </p>
          )}

          <div
            className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold font-display tracking-wide
              ${isConnected ? "bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/25" : "bg-rose-500/10 text-rose-400 ring-1 ring-rose-500/25"}`}
          >
          ...
          <button
            onClick={onRefresh}
            className="group flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800/60 ring-1 ring-slate-700/50 text-slate-400 hover:bg-slate-700/60 hover:text-cyan-400 transition-all duration-200"
